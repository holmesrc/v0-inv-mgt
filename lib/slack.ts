...
export function createLowStockAlertMessage(items: any[]) {
  ...
  displayItems.forEach((item) => {
    ...
    const baseUrl = `${process.env.NEXT_PUBLIC_APP_URL || "https://v0-inv-mgt.vercel.app"}/api/slack/purchase-request`;
...
export function createTextOnlyLowStockAlert(items: any[]) {
  ...
  if (remainingCount > 0) {
    message += `_...and ${remainingCount} more items need attention_\n\n`
    message += `To see all low stock items, click here: <${process.env.NEXT_PUBLIC_APP_URL || "https://v0-inv-mgt.vercel.app"}/low-stock|View All Low Stock Items>\n\n`
  }
...
// Helper function to get item details for purchase requests
export function getItemPurchaseDetails(item: {
  partNumber: string
  description: string
  currentStock?: number
  reorderPoint?: number
...
// This function now returns a URL to our API endpoint that will send the request to Slack
export function createPrefillPurchaseRequestUrl(item: {
  ...
  const details = getItemPurchaseDetails(item);
